...
class ExpenseEntryDialog(tk.Toplevel):
    def __init__(self, parent, on_save, existing_expense: Expense = None):
        ...
        self.__amount_var = tk.DoubleVar()
    ...
    def _create_widgets(self):
        ...
        self.__category_entry = ttk.Entry(self.__main_frame, textvariable=self.__category_var, width=30)
        ...
        self.__amount_entry = ttk.Entry(self.__main_frame, textvariable=self.__amount_var, width=30)
        ...
        self.__description_entry = ttk.Entry(self.__main_frame, textvariable=self.__description_var, width=30)
    ...
    def _load_existing_data(self):
        if self.__existing_expense:
            self.__category_var.set(self.__existing_expense.get_category())
            self.__amount_var.set(self.__existing_expense.__int__())
            self.__description_var.set(self.__existing_expense.get_description())

    def _save_expense(self):
        category = self.__category_var.get().strip()

        try:
            amount_value = self.__amount_var.get()
        except:
            amount_value = None

        description = self.__description_var.get().strip()
